...
Published or bundled source matrices are loaded separately from model age-grid
adaptation. `load_contact_matrix_source()` returns a validated
`agepi_contact_matrix_source` on its native `AgeStructure()`, with top-level
provenance fields such as `source`, `country`, `setting`, `orientation`,
`source_reference`, notes, limitations, and additional metadata. Available
source values are:

- `polymod` / `polymod_uk`: POLYMOD survey matrices through optional
  `socialmixr`. `polymod_uk` is a compatibility alias for the United Kingdom.
- `prem`: Prem et al. 2017/2021 synthetic matrices through optional
  `contactdata`, with country, setting, geographic-setting, and data-source
  selection where the data package supports them.
- `conmat`: matrices generated by optional `conmat` from caller-supplied
  population data. This is not a country loader because conmat generation
  depends on the supplied demography and modelling defaults.
...
Direct `source = "conmat"` support therefore requires user-supplied population
data and delegates generation to `conmat::extrapolate_polymod()`. If callers
need a different conmat model fit or prediction workflow, they should run that
workflow upstream and pass the resulting long table or matrix through
`contact_matrix_from_conmat()` / `as_agepi_contact_matrix()`.
